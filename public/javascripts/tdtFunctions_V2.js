...
var CSB = {
	...
	setTJYBContent(pois){
		...
		for (var i = 0; i < pois.length; i++) {console.log(pois[i]['district']);
			pois[i]['district'] = pois[i]['district'] == null ? ' ' : pois[i]['district'];
...
var MNSL = {
	...
	//预计算各类导航应跳转的连接
	precomputation_Baidu: null,
	precomputation_IOS: null,
	precomputation_Ali: null,
	precomputation_Qq: null,
	...
	callExternalMaps: function(mapType){
		if (this.type == 'navi') {
			MNSL.callExternalMapsNavi();
		}else if(this.type == 'mark'){
			MNSL.callExternalMapsMark();
		}else{
			console.warn('出现未知的操作类型！');
		}
	},

	callExternalMapsMark: function(){
		//百度
	...
	//跳转至地图页面
	gotoMap: function(mapType){
		switch(mapType){
			case '百度':
				window.open(this.precomputation_Baidu);
				break;
			case '苹果':
				window.open(this.precomputation_IOS);
				break;
			case '高德':
				window.open(this.precomputation_Ali);
				break;
			case '腾讯':
				window.open(this.precomputation_Qq);
				break;
			default:
