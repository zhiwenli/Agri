function createGoogleMaps(Cesium,options){
	...
	// MapTypeId.TERRAIN 基于地面信息显示物理地图。
        ...
        var myMapTypeID =options.maptype;
        switch (myMapTypeID)
	{
	       case "roadmap"://道路地图			
    			this._url = "http://maps.googleapis.com/maps/api/staticmap?maptype=roadmap&center={y},{x}&zoom={level}&size=256x256&key={key}";
  			break;

               case "hybrid":	 //正常视图和卫星视图		
    			this._url = "http://maps.googleapis.com/maps/api/staticmap?maptype=hybrid&center={y},{x}&zoom={level}&size=256x256&key={key}";
  			break;

               case "terrain":  //基于地面信息显示物理地图		
    			this._url = "http://maps.googleapis.com/maps/api/staticmap?maptype=terrain&center={y},{x}&zoom={level}&size=256x256&key={key}";
  			break;
