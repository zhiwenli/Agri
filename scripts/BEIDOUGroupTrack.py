...
#实体packet
def object_packet(sat_name,sat_orbit,sat_mission_date,line2,cartesian_data):
   ...
   +"</p><p>Satus: In operation</p>"+"</p> <p>Orbit: "+sat_orbit+"</p><p>Owner: China</p>"\
   +"<p>Mission date: "+sat_mission_date+"</p>"
    ...
    "model": {
        "gltf" : "/data/permanent/satellite_glb_model/BEIDOU.glb",
        "scale" : 1,
        "minimumPixelSize": 30
    },
	 "billboard":
     {
      ...
      "image":"/public/img/BEIDOUbillboard.png"
    },
	  "label":{
      "style" : "FILL_AND_OUTLINE",
      "outlineWidth": 0.5,
      "horizontalOrigin": "LEFT",
      "pixelOffset": {"cartesian2": [35,-35]},      
      "text": sat_name,
      "show": True,
      "outlineColor": {"rgba": [255,215,0,255]},
      "font": "10pt TimesNR",
...
def readTLEfile():
    ...
    linelist = open(CurrentPath+"/BEIDOU_TLE.txt",'r').readlines()
    #删除重复的 卫星名，每两行表示一颗碎片
...
def EverySAT(every_satellite_TLE):
    ...
    sat_orbit=every_satellite_TLE[0].split("(")[1].split(",")[0]
    ...
    #模型版czml
    object_packet_string=object_packet_string+',\n'+object_packet(sat_name,sat_orbit,sat_mission_date,line2,cartesian_data)
